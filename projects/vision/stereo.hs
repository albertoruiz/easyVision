...
plotpoints (x1,y1) (x2,y2) title l = do
    ...
    gnuplot $ "set size ratio -1; plot "++rg x1 x2++" "++rg y1 y2++" \"-\" with points title '"++title++"'\n"
              ++ prep l
...
stereo xwidth ywidth datafile = do    
    ...
    let Just m' = selectCamera' (head pts) (head pts') m ms
...
rectif xwidth ywidth datafile = do    
    correspondences <- readMatrix `fmap` readFile datafile
    let raw  = toLists $ takeColumns 2 correspondences
    let raw'  = toLists $ dropColumns 2 correspondences
--    plotpoints (0,ywidth) (xwidth,0) "first view" raw
...
--    plotpoints (1,-1) (-1,1) "second view" pts'
    ...
    putStrLn "compatible projective cameras:"  
    let (m,m') = canonicalCameras f  
    disp 4 m
    print $ rank m  
    disp 4 m'
    print $ rank m'
    print $ rank (takeColumns 3 m')  
      
    putStrLn "epipoles:"  
    let (ep,ep') = epipoles f
    print $ toList $ inHomog ep
    print $ toList $ inHomog ep'
--    plotpoints (2,-2) (-2,2) "epipole first view" $ (toList $ inHomog ep) : pts
--    plotpoints (2,-2) (-2,2) "epipole second view"$ (toList $ inHomog ep') : pts'
    ...
    gnuplot "set size ratio -1; plot [1:-1] [1:-1] 'dispar.txt' with vectors"
    system "rm dispar.txt"

    
main = do
    --stereo 640 640 "corrs.txt"
    rectif 640 480 "corrs.txt"
