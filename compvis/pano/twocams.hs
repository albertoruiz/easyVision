...
main = do
    ...
    (cam0,ctrl0) <- getCam 0 sz >>= withChannels >>= monitorizeIn "cam0" (mpSize 5) (drawImage.gray) >>= addDist >>= withPause
    (cam1,ctrl1) <- getCam 1 sz >>= withChannels >>= monitorizeIn "cam1" (mpSize 5) (drawImage.gray) >>= addDist >>= withPause
